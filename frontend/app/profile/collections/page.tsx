...
export default function CollectionsPage() {
  ...
  const handleCreateCollection = async () => {
    if (!newCollectionName.trim()) {
      // OLD: toast.error('Назва колекції не може бути порожньою.');
  ...
  const handleDelete = (e: React.MouseEvent, id: number, name: string) => {
    e.stopPropagation();
    // OLD: if (window.confirm(`Ви впевнені, що хочете видалити колекцію "${name}"? Цю дію неможливо скасувати.`)) {
    ...
    <div className="container mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
            ...
            {/* OLD: <h1 className="text-2xl font-bold">Мої колекції</h1> */}
            <h1 className="text-2xl font-bold">{t('profilePages.collections.pageTitle')}</h1>
        </div>
        <button
      ...
      {isInitialized && collections.length === 0 && (
        <div className="text-center py-12 text-gray-500 dark:text-gray-400 bg-white dark:bg-slate-800 rounded-2xl">
          ...
          {/* OLD: <h2 className="text-xl font-semibold mb-2">У вас ще немає колекцій</h2> */}
          <h2 className="text-xl font-semibold mb-2">{t('profilePages.collections.empty.title')}</h2>
          {/* OLD: <p className="mb-4">Створіть свою першу колекцію, щоб зберігати улюблені товари.</p> */}
          <p className="mb-4">{t('profilePages.collections.empty.subtitle')}</p>
          <button
            onClick={() => setShowCreateModal(true)}
            className="px-5 py-2.5 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-colors"
          >
            {/* OLD: Створити першу колекцію */}
          ...
          {collections.length < 9 && (
            ...
            >
              <div className="text-center">
                ...
                {/* OLD: <h3 className="font-semibold">Створити колекцію</h3> */}
                <h3 className="font-semibold">{t('profilePages.collections.createCta')}</h3>
              </div>
            </motion.div>
          )}
        </div>
      ...
      <AnimatePresence>
        {showCreateModal && (
          ...
          >
            ...
            >
              {/* OLD: <h2 className="text-lg font-bold mb-4">Створити нову колекцію</h2> */}
              <h2 className="text-lg font-bold mb-4">{t('profilePages.collections.modal.title')}</h2>
              <input
                type="text"
                // OLD: placeholder="Назва колекції (напр., 'Кухня')"
              ...
              />
              <div className="mb-4">
                {/* OLD: <p className="text-sm font-medium mb-2">Колір іконки</p> */}
                <p className="text-sm font-medium mb-2">{t('profilePages.collections.modal.iconColor')}</p>
              ...
              <div className="flex gap-2 mt-6">
                {/* OLD: <button onClick={handleCreateCollection} className="flex-1 px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600">Створити</button> */}
                <button onClick={handleCreateCollection} className="flex-1 px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600">{t('common.create')}</button>
                {/* OLD: <button onClick={() => setShowCreateModal(false)} className="flex-1 px-4 py-2 bg-gray-200 dark:bg-slate-700 rounded-lg hover:bg-gray-300 dark:hover:bg-slate-600">Скасувати</button> */}
                <button onClick={() => setShowCreateModal(false)} className="flex-1 px-4 py-2 bg-gray-200 dark:bg-slate-700 rounded-lg hover:bg-gray-300 dark:hover:bg-slate-600">{t('common.cancel')}</button>
              </div>
            </motion.div>
          </motion.div>
        )}

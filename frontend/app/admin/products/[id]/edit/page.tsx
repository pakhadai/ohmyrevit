...
export default function EditProductPage() {
  ...
  // Функція для завантаження даних товару
  const fetchProduct = async () => {
    try {
      const response = await api.get(`/products/${productId}`, {
        headers: { 'Accept-Language': 'uk' } // Завжди запитуємо українську версію для редагування
      })
  ...
  const handleSubmit = async () => {
    if (!formData.title_uk || !formData.description_uk) {
      toast.error("Будь ласка, заповніть поля назви та опису")
      return
    }
  ...
  const handleDelete = async () => {
      if (window.confirm(`Ви впевнені, що хочете видалити товар #${productId}? Цю дію неможливо буде скасувати.`)) {
          setLoading(true);
          try {
  ...
  // Функції для роботи з галереєю та категоріями (аналогічні до NewProductPage)
  const addGalleryImage = () => {
    const url = prompt('Введіть URL зображення:')
    if (url) {
      setFormData({ ...formData, gallery_image_urls: [...formData.gallery_image_urls, url] })
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          {/* ... (вміст форми, аналогічний до NewProductPage) ... */}
           {/* Основна інформація */}
          <div className="mb-8">
            ...
            <div className="mt-4">
              ...
              <textarea value={formData.description_uk} onChange={(e) => setFormData({ ...formData, description_uk: e.target.value })} className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600" rows={5} required />
            </div>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              {formData.is_on_sale && (
                <input type="number" step="0.01" min="0" placeholder="Ціна зі знижкою" value={formData.sale_price || ''} onChange={(e) => setFormData({ ...formData, sale_price: e.target.value ? Number(e.target.value) : null })} className="px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600" />
              )}
            </div>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="url" value={formData.main_image_url} onChange={(e) => setFormData({ ...formData, main_image_url: e.target.value })} className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600" required />
                {formData.main_image_url && <img src={formData.main_image_url} alt="Preview" className="mt-2 w-full h-32 object-cover rounded-lg border dark:border-gray-600" />}
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">ZIP файл (шлях) *</label>
                <input type="text" value={formData.zip_file_path} onChange={(e) => setFormData({ ...formData, zip_file_path: e.target.value })} className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600" required />
                <label className="block text-sm font-medium mb-2 mt-4">Розмір файлу (MB) *</label>
                <input type="number" step="0.01" min="0" value={formData.file_size_mb} onChange={(e) => setFormData({ ...formData, file_size_mb: Number(e.target.value) })} className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600" required />
              </div>
            </div>
            <div className="mt-4">
              <label className="block text-sm font-medium mb-2">Галерея зображень</label>
              <div className="flex flex-wrap gap-2 mb-2">
                {formData.gallery_image_urls.map((url, index) => (
                  <div key={index} className="relative group"><img src={url} alt={`Gallery ${index + 1}`} className="w-20 h-20 object-cover rounded-lg border dark:border-gray-600" /><button type="button" onClick={() => removeGalleryImage(index)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"><X size={12} /></button></div>
                ))}
        ...
        <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <p className="text-sm text-blue-700 dark:text-blue-300"><strong>Примітка:</strong> При зміні назви або опису автоматично оновиться переклад на інші мови. Ви також можете вручну відредагувати переклади в майбутньому розділі "Переклади".</p>
        </div>
      </div>
    </div>
  )
